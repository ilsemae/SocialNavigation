...
def get_net_force(turtle_name,goal):
	...
	for stranger in ["theo","josephine","yma","alan","everett","rose","zed","robin"]:
		...
		if stranger!=turtle_name:	# There is no force on that turtle from itself.
			...
			goal_coefficient = goal_coefficient + 30  # for each stranger turtle, the weight of the goal must increase to keep it proportionally important enough
	...
	#print(turtle_name+": due to goal:"+str(30*goal_vect))
...
def driver(turtle_name):
	...
	raw_input() # wait for user to hit enter to go
	...
	while not rospy.is_shutdown():
		...
		# if the turtle has reached its goal, kill this node
...
def mommy(baby_name):
	...
	try:
		...
		if side==0:
			x = 0
			theta = 0
			rospy.set_param(baby_name+"_goal","right")
		else:
			x = 11
			theta = np.pi
			rospy.set_param(baby_name+"_goal","left")
		y = float(np.random.randint(15*(wall_y_positions[0]+.5),15*(wall_y_positions[1]-.5)))/15		
